module SimpleTransformer.ImplV5
  ( StateT(..)
  , get
  , put
  , lift
...
get = let newStateFunc s = pure (s, s) in StateT { runStateT = newStateFunc }

put :: (Monad m) => s -> StateT s m ()
put s =
  let newStateFunc _ = pure ((), s) in StateT { runStateT = newStateFunc }
...
instance Applicative m => Applicative (StateT s m) where
  pure x =
    let newStateFunc s = pure (x, s) in StateT { runStateT = newStateFunc }
  stf <*> sta =
